...
public class AuthController {
    ...
    public String register(@Valid @ModelAttribute("user") UserDto userDto,
        ...
        if (!isUsernameAvailable) {
            bindingResult.rejectValue("username", "error.user", "Потребителското име е заето");
        }
        if (!isEmailAvailable) {
            bindingResult.rejectValue("email", "error.user", "Имейлът вече е регистриран");
        }
        ...
        return "redirect:/auth-microservice/auth/login";
    }
}